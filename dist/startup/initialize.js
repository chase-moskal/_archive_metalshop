...
export async function initialize(config) {
    ...
    //
    if (config.mock !== null) {
        const { makeMetalMocks } = await import("./make-metal-mocks.js");
    ...
    //
    const operations = [];
    const queue = (func) => operations.push(func());
    const { ["auth-server"]: authServerOrigin, ["paywall-server"]: paywallServerOrigin, ["profile-server"]: profileServerOrigin, ["liveshow-server"]: liveshowServerOrigin, ["schedule-server"]: scheduleServerOrigin, ["questions-server"]: questionsServerOrigin, } = config;
    if (authServerOrigin) {
        queue(async () => {
