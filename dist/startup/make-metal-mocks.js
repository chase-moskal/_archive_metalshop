...
export const makeMetalMocks = async ({ startAdmin, startPremium, startLoggedIn, logger = console, googleUserName = "Steve Stephenson", decodeAccessToken = defaultDecodeAccessToken, googleUserAvatar = "https://i.imgur.com/CEqYyCy.jpg", generateRandomNickname = () => `User-${random8().toUpperCase()}` }) => {
    ...
    const triggerAccountPopup = async () => authExchanger.authenticateViaGoogle({ googleToken });
    const triggerCheckoutPopup = async ({ stripeSessionId }) => null;
    //
    // starting conditions
    //
    ...
    if (startLoggedIn || startAdmin || startPremium) {
        ...
        if (startAdmin) {
            const { user } = await verifyToken(authTokens.accessToken);
...
//# sourceMappingURL=make-metal-mocks.js.map